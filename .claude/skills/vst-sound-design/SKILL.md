...
- **Serum 2** : rien d'exposé (sauf ce que l'utilisateur mappe en mode Configure). Chargement par `lom.py load "<piste>" "Serum 2"` (la règle anti hot-swap est appliquée et vérifiée ; VST3 et AU coexistent → préciser le chemin si ambigu) — sauf quand on VEUT remplacer l'instrument de la piste : `lom.py load "<piste>" "Serum 2" replace=<ancien instrument>` = remplacement en place vérifié, chaîne d'effets conservée. Réglages dans la fenêtre : voir `references/serum2.md`.
...
4. Mesurer : `lom.py meters <mesure>|1 5 "<piste>"` (ou `levels.sh`) pour comparer au niveau de l'ancien son (crête post-fader, relatif) ; `lom.py snapshot` avant de retoucher un réglage ; compenser sur un Utility ou le volume de l'instrument, pas sur un fader automatisé.

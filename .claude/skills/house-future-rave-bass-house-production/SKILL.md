...
## Passer la main
Un son seul hors de ces recettes → `sound-designer-serum` puis `vst-sound-design` ; grille harmonique ou mélodie à écrire → `compositeur-arrangeur` et `theorie-musicale-electronique` ; batterie et groove → `producteur-rythmique` et `drums-signature` ; minutage, repères, Insérer Silence → `arrangement-avance` ; le grave se bat → `kick-bass-equilibre` ; masquage, diagnostic → `ingenieur-mixage` ; LUFS, true peak, livraison → `live-mix-mastering`, `mastering-outils`, `live-export-wav` ; référence audio à analyser → `synthese-reference` ; cuivres de festival ou horn stabs → `studio-grade-brass-sound-design` ; vélocités et gate → `midi-expressif`.

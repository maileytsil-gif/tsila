...
2. **Relire l'état réel** avant d'agir : carte des clips (`../arrangement-avance/scripts/arrangement_map.py`) ou `scripts/snapshot_clips.py` ; comparer avec la mémoire, noter les écarts (« l'utilisateur a dupliqué le hook sur 13–17 et 21–25 »).
3. Vérifier `int(song.is_playing)` et l'heure du dernier `.als` sauvé.
...
- Avant toute transformation de notes (re-voicing, transposition, réécriture) : `scripts/snapshot_clips.py` via `pyl.sh` → JSON daté ; après : `scripts/diff_snapshot.py avant.json apres.json` pour vérifier que seules les pistes visées ont bougé.
...
1. Sauver le Set (Fichier › Sauver Set Live) et noter l'heure du fichier.
...
## Revenir en arrière
- Live écrit une copie horodatée à chaque sauvegarde dans `<dossier projet>/Backup/<Set> [AAAA-MM-JJ HHMMSS].als`. `scripts/als_notes.py <fichier.als> <sortie.json> [PISTE …]` extrait les notes visibles des clips d'arrangement (temps absolus) — sans ouvrir le Set. Réinjecter avec `remove_notes_extended` + `add_new_notes` dans le clip existant (ne pas supprimer le clip : l'automation de piste échantillonnée par le bridge y est attachée).

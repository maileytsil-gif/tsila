...
description: Méthode de travail pour produire dans Ableton Live 12 avec Producer Pal, le LOM Bridge (lom.py) et le contrôle d'écran, telle que cet utilisateur la pratique, et **carte d'entrée vers les 29 autres skills musique** (situation → chaîne de skills). Utilise ce skill dès que la demande touche Ableton, un Set Live, une piste, un clip, un device, un plug-in (Serum, Waves, FabFilter, iZotope…), un bus, un niveau, une sauvegarde ou une vérification dans Live — même si l'utilisateur n'écrit que « la basse », « le drop », « le master » ou « corrige ». Utilise-le aussi en tout premier quand l'utilisateur commence un NOUVEAU morceau (« nouveau projet », brief de style/BPM/tonalité) : il donne l'ordre des 8 étapes et les règles apprises à la dure (hot-swap, transport, relecture, sauvegarde, natifs tolérés, une étape par échange).
---
...
| Future rave, bass house, tech house, future house, big room, house de club : un titre entier, un drop, une basse de genre, un master club (références Guetta, MORTEN, Garrix, Chris Lake, Jauz, Dom Dolla) | house-future-rave-bass-house-production (grille et courbe d'énergie, recettes, deux masters, corpus local `corpus/house-future-rave/`) → arrangement-avance → kick-bass-equilibre → sound-designer-serum / vst-sound-design → ingenieur-mixage → live-mix-mastering → live-export-wav |
| Techno, minimal/deep tech, melodic house/techno, afro house, melodic dubstep, liquid ou minimal DnB ; robotic bass, vocoder ; reverse, pré-FX, suckback ; QC « label-ready », stems, archive ; APC64, Komplete Kontrol A49, Maschine 3, Analog Obsession | electronic-production-engineer (skill en anglais, v1.5.0 : phases brief → archive, boucle diagnostic → une modification à la fois, garde de capacité, inventaire du studio, 10 ADN de genres, contrat du bridge en `plan_only` avec approbation liée au plan exact, schémas JSON validés ; Qwen : `ollama/run_skill.py`) → la ligne spécialisée ci-dessus pour exécuter (sound-designer-serum, ingenieur-mixage, live-mix-mastering…) |

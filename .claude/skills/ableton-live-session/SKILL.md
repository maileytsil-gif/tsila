...
description: Méthode de travail pour produire dans Ableton Live 12 avec Producer Pal, le LOM Bridge (lom.py) et le contrôle d'écran, telle que cet utilisateur la pratique, et **carte d'entrée vers les 28 autres skills musique** (situation → chaîne de skills). Utilise ce skill dès que la demande touche Ableton, un Set Live, une piste, un clip, un device, un plug-in (Serum, Waves, FabFilter, iZotope…), un bus, un niveau, une sauvegarde ou une vérification dans Live — même si l'utilisateur n'écrit que « la basse », « le drop », « le master » ou « corrige ». Utilise-le aussi en tout premier quand l'utilisateur commence un NOUVEAU morceau (« nouveau projet », brief de style/BPM/tonalité) : il donne l'ordre des 8 étapes et les règles apprises à la dure (hot-swap, transport, relecture, sauvegarde, natifs tolérés, une étape par échange).
---

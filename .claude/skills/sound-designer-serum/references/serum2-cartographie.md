...
| Étiquette | Sens |
|---|---|
| `[DOC]` | Manuel officiel Serum 2 (354 pages, dépouillé dans `serum2-fx-clip-arp.md`, `moteurs-synthese.md`, `serum2-automation-et-migration.md`), manuel Serum 1 (`../../../../corpus/constructeur/xfer-serum-1-manuel-1-0-1.md`), changelog Xfer (`../../../../corpus/constructeur/xfer-serum-changelog-2-0-24-a-1-10.md`) |
...
- **Bas droite** : **MACRO 1–8**, **voicing** (MONO (1058, 627), LEGATO (1058, 653), POLY) et **portamento** (PORTA (1081, 720), curve, Always, Scaled) `[LOCAL]` `[DOC]`.
...
| Pitch track | on/off | on | `kParamPitchTrack` | Off : toutes les notes à la même hauteur (drums, bruit) |
| Mode de hauteur | Ratio · Harmonics | — | `kParamPitchMode`, `kParamPitchRatio` −1…24, `kParamHzOffset` −1000…+500 Hz | Accordage façon opérateur FM `[USINE]` ; libellé exact du panneau **MUET** |
...
- **WT POS** 1–256 (`kParamTablePos`), la position dans la table ; clic droit → « Smooth Interpretation » (sic, = interpolation lisse, quasi-infinie entre frames). Interpolation entre frames : crossfade ou **spectral morphing**, choisie dans l'éditeur, calculée au chargement.
...
| Affichage | clic droit : Frequency Response · + FFT · Phase Response & FFT | | |
...
| **Delay** | 4 `FXDelay` | MODE Normal / Ping-Pong / Tap→Delay · temps L/R (`kParamTimeL/R`, sync : fast … 4 bar) + **offset** L/R 0,5–1,5 (« Trip » à 133 %, « Dot » à 150 %) · LINK · FEEDBACK 0–95 · FREQ 40–18 000 Hz + BW 0,75–8,25 (**Q inversé**) · HQ · BPM · MIX ; double-clic sur l'affichage = overlay de fréquence |
| **Distortion** | 0 `FXDistortion` | TYPE (16 : Tube, Soft Clip, Hard Clip, Diode 1, Diode 2, Lin. Fold, Sin Fold, Zero-Square, Downsample, Asym, Rectify, X-Shaper, X-Shaper (Asym), Sine Shaper, Stomp Box, Tape Sat. ; Serum 2 ajoute **Overdrive**, Soft Sat., et un **DC bias** `[EXTRAIT]`) · DRIVE 0–100 (en Downsample = taux d'échantillonnage, en X-Shaper = morph A→B) · filtre OFF / PRE / POST avec type morphable LP→BP→HP (`kParamLPHP`), FREQ (8–13 290 Hz, clic droit Key Track), Q (`kParamBW`) · stages (`kParamNumStages` 2–16) · MIX |
...
Structure `[DOC]` `[BIN]` : racks MAIN, BUS 1, BUS 2 ; flux du haut vers le bas ; **jusqu'à 16 slots par rack** (`FX: array[max 16 slots]`) ; chaque module a MIX (sauf EQ) et LEVEL (`kParamLevelOut`, trim de sortie), bouton bypass rouge (Alt-clic = tout le bus), vue étendue Alt-F ; clic droit sur le fond : Cut/Copy/Paste/Clear, **Lock FX Bus** (garde les modules au changement de preset **mais efface les modulations vers ce rack**), Load/Save FX Bus. Ordre d'usine Serum 1 : Hyper en premier. Rack par défaut « Serum FX » : Serum 2 existe aussi en **plug-in d'effet** (`product: Serum2FX`, deux presets d'usine « SerumFX - … ») `[USINE]`.
...
- **TRANSPOSE** (demi-tons, toutes notes entrantes et générées) · **KEY** · **SCALE** (les notes hors gamme sont ramenées dans la gamme, surtout pour CLIP et ARP ; `PitchQuantizer0`) · **SWING** (global, OFF par défaut, division qui apparaît dès qu'on quitte OFF ; plage selon l'hôte : **Live 12,5–87,5 %**, FL −150…+150 %) `[DOC p. 269]`.
- **OSC Mapping** : éditeur qui assigne chaque oscillateur à une zone de clavier (par exemple SUB + A dans le grave, B au médium, C au-dessus de C5) et de vélocité ; c'est `kParamKeyZoneMin/Max`, `kParamVelocityZoneMin/Max` par oscillateur `[BIN]`. Tag d'usine « KB-Span » (17 presets).
...
Icône liste (1010, 36) ; arbre Factory › Arp / Bass / Bell / Chord / Drum / FX / Keys / Lead / Pad / Pluck / Seq / Synth / Vox … ; recherche par nom et **tags** ; filtres catégorie, genre, tags perso, **notes en étoiles** ; **autoplay** d'une mélodie par défaut ou d'un **clip de prévisualisation** fourni par le sound designer (clic droit sur un clip → Set as Preview Clip). Tags stockés dans le fichier : `[badge du moteur (Wavetable / Multisample / Sample / Granular / Spectral), Poly | Mono, catégorie (Arp / Clip / Embedded-Data / KB-Span / Custom-Tuning), Preview]`. Presets utiles sur ce Mac : Drum › « DR - Kick Minimal », Bass › Electric / Sub, Keys / Pad ; banques perso TKNVLT, « Kick Future House Tsila », F Brooks `[LOCAL]`. Verrous : FX bus, oversampling, tuning (état non restauré par l'hôte avant la préférence 2.1.5).
...
- **Où compléter** : le manuel officiel (354 pages, https://xferrecords.com/manual/serum-2/docs et la version web) est la seule source qui tranche les MUET ; les pages `using-the-serum-keyboard`, `using-knobs-and-sliders`, `resizing-the-ui` sont listées dans `../../../../corpus/sources-a-telecharger.json` pour le script `fetch_sources.py` du Mac.

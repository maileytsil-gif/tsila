...
| `tests/test_offline.py` | 56 tests logiciels sans Live (`python3 -W ignore -m unittest tests/test_offline.py`) |
...
- **Journal** : chaque commande qui modifie le Set (`/shape`, `/clear`, `/setparam`, `/restore`, `/locator`, `/transport`) ajoute une ligne JSON à `~/Library/Application Support/LOMBridge/journal.jsonl` (heure, session, version, id, commande, arguments, lignes de résultat ou erreur + code). `/journal [n]` relit les n dernières.

...
| `tests/test_offline.py` | 62 tests logiciels sans Live (`python3 -W ignore -m unittest tests/test_offline.py`) |
| `tests/live_suite.py` | Suite d'essais dans Live sur deux pistes temporaires créées puis supprimées |
| `SESSION-MORCEAU-CLAUDE-CHATGPT.md` | Protocole de session commune (deux agents, même Set, même bridge), issu de la revue ChatGPT |
...
- Réponses, l'id de requête étant **toujours le premier argument** : `/begin <id> <cmd>` · `/r <id> …` (0 à n lignes ; certaines commencent par `warn`, `error`, `plan`, `clip`, `rebuilt`) · `/err <id> <code> <texte>` · `/end <id> <cmd>`. Réponses envoyées à l'adresse de l'expéditeur. Le client ne garde que les lignes portant son id.
- **Codes d'erreur stables** (`<code>` de `/err`, aussi dans `codes` côté client, `[E_…]` dans le journal d'`apply`) : `E_AUTH`, `E_UNKNOWN_CMD`, `E_USAGE`, `E_QUEUE_FULL`, `E_CANCELLED`, `E_TRANSPORT_PLAYING`, `E_TRANSPORT_STOPPED`, `E_CURSOR_MOVED`, `E_STALE` (replanifier), `E_PLAN`, `E_AUTOMATION_OVERRIDDEN`, `E_AUTOMATED`, `E_REF`, `E_NOT_FOUND`, `E_AMBIGUOUS`, `E_ACCEPT`, `E_NOT_APPLIED`, `E_VERIFY`, `E_ROLLED_BACK` (étape défaite, rien n'est modifié), `E_ROLLBACK_FAILED` (faire Cmd+Z), `E_UNCERTAIN` (fin d'étape d'annulation impossible : vérifier dans Live avant toute autre écriture), `E_RANGE`, `E_LIMIT`, `E_INVALID`, `E_ERROR` ; `E_TIMEOUT` côté client. Le texte reste la description pour l'humain.
- **Journal** : chaque commande qui modifie le Set (`/shape`, `/clear`, `/setparam`, `/restore`, `/locator`, `/transport`, `/load`, `/notes`) ajoute une ligne JSON à `~/Library/Application Support/LOMBridge/journal.jsonl` (heure, session, version, id, commande, arguments, lignes de résultat ou erreur + code). `/journal [n]` relit les n dernières.
...
- Temps : noires absolues depuis 1|1 (le client accepte `17|1`, `17|3.5`, `5|2|3` ; signature lue dans Live, `--bpb` en CLI ou `beatsPerBar` dans le corps HTTP pour forcer).
...
| `/journal [n]` | `entry <json>` × n (plus ancienne d'abord) puis `journal <fichier> <total>` |
| `/load <piste> <nom> [source] [replace=<device>]` | charge un device par le navigateur de Live **sans hot-swap** : la piste est sélectionnée et son dernier device aussi (ou celui de `replace=`), puis `load_item` ; contrôle après : +1 device en fin de chaîne (ou même nombre avec `replace`), aucun device existant remplacé, aucune autre piste modifiée — sinon erreur `E_NOT_APPLIED` et consigne Cmd+Z. Nom exact, sinon sous-chaîne unique (« Serum 2 » présent en VST3 et AU → ambigu : préciser). `source` : `plugins` (défaut), `sounds`, `instruments`, `audio_effects`, `midi_effects`, `drums`, `max_for_live`, `user_library`. → `loaded <ref> <nom> <index> <n avant> <n après> <chemin> added\|replaced` |
| `/notes get <piste> <clipRef\|t> [tA tB]` | `clip <ref> <nom> <début> <fin> <start_marker> <n>` puis `note <pitch> <début> <durée> <vélocité> <mute>` — temps **relatifs au clip** (comme Live, C3 = 60) |
| `/notes set <piste> <clipRef\|t> <json> [tA tB]`, `/notes add …` | `set` remplace toutes les notes du clip, ou celles de la fenêtre `[tA, tB[` (une note écrite hors fenêtre est refusée) ; `add` ajoute. JSON : `[{"pitch":60,"start":0,"duration":1,"velocity":100,"mute":false}]` ou `[[60,0,1,100]]`. Une étape d'annulation, relecture note à note, **défaite si Live n'a pas écrit exactement** ce qui était demandé. Le clip n'est jamais supprimé (l'automation y reste attachée) |
...
- **0.8.2** (revue ChatGPT) : `/notes` marque le clip comme touché **avant** chaque appel à Live → une écriture partielle (`add_new_notes` qui lève à mi-chemin) est défaite ; `end_undo_step` qui lève → `E_UNCERTAIN`, pas d'undo à l'aveugle. Protocole de session commune Claude/ChatGPT : `SESSION-MORCEAU-CLAUDE-CHATGPT.md`. 2 tests.
...
- **0.7.0** : codes d'erreur stables (table testée), `/err` à trois arguments lu par le client, journal des écritures (lectures non journalisées, échec journalisé avec son code, tâche asynchrone journalisée à sa fin), `policy.json`, signature lue dans Live (`3/4` → `3|1` = 6 temps). 4 tests hors Live ; § 9 de `live_suite.py` à passer dans Live.
